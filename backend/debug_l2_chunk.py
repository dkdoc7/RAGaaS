...
async def main():
    query = "오영수 배우의 배역은?"
    # The chunk user mentioned
    chunk_content = "서울대학교에서 공부했다. 고객으로부터 돈을 훔치고 잘못된 투자로 막대한 빚을 지게 된 경찰을 피해 게임에 참여한다. (시즌 1) ● 위하준 - 황준호 역: 실종된 형을 찾기 위해 게임에 잠입한 경찰. ● 오영수 - 오일남 역: 1번 참가자. 뇌종양에 걸린 칠순 노인으로 치매 증상이 있다."
    
    print(f"Query: {query}")
    print(f"Chunk Snippet: {chunk_content[:100]}...")
