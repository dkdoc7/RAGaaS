...
[쿼리 작성 원칙]
1. 엔티티 검색: 질문에 언급된 개체는 `label_ko` 속성을 사용하여 매칭하세요.
   (예: `MATCH (n:Entity {label_ko: "성기훈"})`)
2. 유연한 매칭: 정확한 이름을 모를 경우 `CONTAINS`를 활용하세요.
   (예: `MATCH (n:Entity) WHERE n.label_ko CONTAINS "기훈"`)
3. 관계 방향 고려: 질문의 의도에 따라 화살표 방향을 정확히 지정하세요. 
   만약 방향이 불확실하다면 방향 없이 검색할 수도 있습니다. (예: `(n)-[:`관계`]-(m)`)
4. 다단계(Multi-hop) 연결: 질문이 여러 단계를 거치는 경우 화살표를 이어 붙이세요.
   (예: `MATCH (n:Entity {label_ko: "성기훈"})-[:`제자`]->(m)-[:`제자`]->(o) RETURN o.label_ko`)
5. 근거(Evidence) 탐색: 만약 질문이 "근거"나 "텍스트"를 요구하면 :Chunk 노드와 연결하세요.
   (예: `MATCH (n:Entity {label_ko: "성기훈"})-[:`MENTIONED_IN`]->(c:Chunk) RETURN c.text`)
6. 결과 형식: `RETURN` 구문을 사용하며, 변수명은 질문의 의도를 잘 반영하도록 지정하세요 (예: n.label_ko AS answer).
...
```json
{
  ...
  "cypher": "생성된 Cypher 쿼리문. 노드의 label_ko 매칭뿐 아니라, 속성값 내 텍스트 포함 여부(CONTAINS)를 검색하는 패턴도 적극 활용하세요.",
...
2. **OR 조건 적극 활용**: 특정 속성 하나만 믿지 말고, 가능한 모든 후보를 `OR`로 연결하여 검색 범위를 넓히세요.
   - 예: `MATCH (n) WHERE n.특징 CONTAINS '장풍' OR n.comment CONTAINS '장풍' OR exists { (n)-[:창시자]-(:Entity {label_ko: '장풍'}) }`
...
4. **양방향 관계 탐색 (필수)**: 자연어 질문의 방향과 DB 저장 방향이 불일치하는 경우가 매우 흔합니다.
   ...
   - 예: '성기훈의 후배' → `MATCH (n:Entity {label_ko: '성기훈'})-[:`후배`]-(m:Entity) RETURN m.label_ko`
...
3. 한국어 지원: `label_ko` 속성을 사용하여 한글 엔티티 명칭을 매칭하세요.
"""
